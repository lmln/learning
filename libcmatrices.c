/* 
   Compile with:
   gcc -shared -fPIC fmatrices.c -o fmatrices.o 

   Or:
   cl -c -DWIN32 libcmatrices.c
...
make_matrix(size_t rows, size_t columns, long double value)
{
  size_t s = rows * columns; 

  struct matrix * new_matrix = malloc(sizeof(struct matrix));
  new_matrix->nums = malloc(sizeof(long double) * s);
...
EXPORT void
matrix_set(struct matrix * matrix, size_t i, size_t j, long double value) 
{ 
...
EXPORT struct matrix * 
scale(long double scalar, struct matrix * mmatrix) 
{ 
  size_t s = mmatrix->rows * mmatrix->columns; 
...
EXPORT struct matrix * 
destructive_scale(long double scalar, struct matrix * mmatrix) 
{ 
  size_t s = mmatrix->rows * mmatrix->columns; 
      ...
      for (size_t j = 0; j < kc; j++) 
        {
          ...
          if(isnan(sum)) {
            display_matrix(mmatrix);
            display_matrix(kmatrix);
            printf("[Detected NAN]\n");
            Sleep(10000);
          }
